...
import com.music.nuvia.data.lyrics.EnhancedLrc
import com.music.nuvia.data.lyrics.LrcLib
import com.music.nuvia.data.lyrics.LyricLine
import com.music.nuvia.data.lyrics.LyricWord
import com.music.nuvia.data.lyrics.LyricsPlus
import com.music.nuvia.data.lyrics.TtmlLyrics
import com.music.nuvia.data.lyrics.toEnhancedLrc
import com.music.nuvia.data.lyrics.toLrc
...
class WordSyncTest {
    ...
    // ---- LRC Writer and Enhanced LRC roundtrips ------------------------------

    @Test
    fun `toLrc formats lines properly and parses back`() {
        val lines = listOf(
            LyricLine(timeMs = 1_500L, text = "First line"),
            LyricLine(timeMs = 4_200L, text = "Second line"),
        )
        val lrc = lines.toLrc()
        val parsed = LrcLib.parseLrc(lrc).sung()
        assertEquals(2, parsed.size)
        assertEquals("First line", parsed[0].text)
        assertEquals(1_500L, parsed[0].timeMs)
        assertEquals("Second line", parsed[1].text)
        assertEquals(4_200L, parsed[1].timeMs)
    }

    @Test
    fun `toEnhancedLrc formats word timings and parses back`() {
        val lines = listOf(
            LyricLine(
                timeMs = 1_000L,
                text = "Hello world",
                words = listOf(
                    LyricWord(startMs = 1_000L, endMs = 1_500L, text = "Hello "),
                    LyricWord(startMs = 1_500L, endMs = 2_000L, text = "world"),
                ),
            ),
        )
        val enhanced = lines.toEnhancedLrc()
        val parsed = EnhancedLrc.parse(enhanced).sung()
        assertEquals(1, parsed.size)
        assertEquals("Hello world", parsed[0].text)
        assertTrue(parsed[0].isWordSynced)
        assertEquals(2, parsed[0].words.size)
        assertEquals("Hello", parsed[0].words[0].text)
        assertEquals(1_000L, parsed[0].words[0].startMs)
        assertEquals("world", parsed[0].words[1].text)
    }

    @Test
    fun `toEnhancedLrc returns empty when lines have no word timings`() {
        val lines = listOf(LyricLine(timeMs = 1_000L, text = "Plain line"))
        assertEquals("", lines.toEnhancedLrc())
    }
}
