...
import com.music.nuvia.download.FlacTagger
import com.music.nuvia.download.LyricsTag
import com.music.nuvia.download.Mp4Tagger
import com.music.nuvia.download.WebmTagger
...
class MediaTaggerTest {
    ...
    fun `flac tagging keeps the frames, carries other blocks and spends the padding`() {
        ...
        assertEquals("NUViA", String(comment, 4, vendorLength, Charsets.UTF_8))
    ...
    @Test
    fun `mp4 tagging embeds lyrics and roundtrips through EmbeddedLyrics`() {
        val (original, _, _) = buildFakeMp4(ByteArray(16))
        val lyrics = LyricsTag.Embeddable(
            plain = "[00:01.00]Hello world",
            enhanced = "[00:01.00]<00:01.00>Hello <00:01.50>world",
        )
        val tagged = Mp4Tagger.tag(
            original, "Title", "Artist", "Album", null, false, lyrics = lyrics
        )
        val extracted = EmbeddedLyrics.fromBytes(tagged)
        assertEquals(lyrics.enhanced, extracted)
    }

    @Test
    fun `flac tagging embeds lyrics and roundtrips through EmbeddedLyrics`() {
        val original = flacMagic + flacBlock(TYPE_STREAMINFO, ByteArray(34), last = true) + ByteArray(16)
        val lyrics = LyricsTag.Embeddable(
            plain = "[00:01.00]Hello world",
            enhanced = "[00:01.00]<00:01.00>Hello <00:01.50>world",
        ...
        val tagged = FlacTagger.tag(
            original, "Title", "Artist", "Album", null, "image/jpeg", lyrics = lyrics
        )
        val extracted = EmbeddedLyrics.fromBytes(tagged)
        assertEquals(lyrics.enhanced, extracted)
    }
    ...
    @Test
    fun `webm tagging embeds lyrics and roundtrips through EmbeddedLyrics`() {
        val unknownSize = byteArrayOf(0x01) + ByteArray(7) { 0xFF.toByte() }
        val segmentBody = ByteArray(10) { (it + 1).toByte() }
        val original = buildFakeWebm(segmentBody, unknownSize)
        val lyrics = LyricsTag.Embeddable(
