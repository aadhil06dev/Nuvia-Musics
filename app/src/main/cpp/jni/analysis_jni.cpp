...
void AppendEnergyCurve(std::string& out, const std::vector<nuvia::smart::EnergyPoint>& points) {
  out += '[';
...
void AppendCuePoints(std::string& out, const std::vector<nuvia::smart::MixCuePoint>& points) {
  out += '[';
  for (size_t index = 0; index < points.size(); ++index) {
    if (index > 0) out += ',';
    out += "{\"t\":";
...
Java_com_music_nuvia_playback_smart_TrackFeatures_nativeAnalyze(
    JNIEnv* env,
    jclass /* clazz */,
    jfloatArray samples,
    jdouble sample_rate,
  ...
  const nuvia::smart::AnalysisResult result =
      nuvia::smart::AnalyzeAudio(input, sample_rate, duration);
...
Java_com_music_nuvia_playback_smart_TrackFeatures_nativeSampleRate(
    JNIEnv* /* env */,
    jclass /* clazz */) {
...
Java_com_music_nuvia_playback_smart_TrackFeatures_nativeResample(
    JNIEnv* env,
    jclass /* clazz */,
    jfloatArray samples,
    jdouble input_rate,
