...
fun ThinSlider(
     ...
     * [com.music.nuvia.data.settings.AppSettings.smartMixInProgress].
     */
    ...
    val resolvedActiveColor = activeColor ?: nuviaColors.primary
    val resolvedInactiveColor = inactiveColor ?: if (nuviaColors.isDark) Color.White.copy(alpha = 0.16f) else Color.Black.copy(alpha = 0.12f)
    val resolvedMarkerColor = markerColor ?: nuviaColors.secondary.copy(alpha = 0.45f)
    ...
    Box(
        modifier = modifier
            .fillMaxWidth()
            // Generous invisible touch target — the visible bar is only ~6-10dp.
        ...
        Canvas(
            ...
            // Draw glass track with subtle top highlight
            drawRoundRect(color = resolvedInactiveColor, cornerRadius = radius)
            ...
            if (filled > 0f && !mixing) {
                val activeWidth = filled.coerceAtLeast(size.height)
                drawRoundRect(
