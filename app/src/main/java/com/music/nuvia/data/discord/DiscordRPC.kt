package com.music.nuvia.data.discord
 ...
 *
 * ```
 *   Listening to NUViA             <- activityName, or the app's own name
 *   ┌────┐  Song title             <- details
 *   │art │  Artist                 <- state
 *   └────┘  ▁▁▁▁▁▁ 1:04 / 3:47     <- from the timestamps
 *   [ Listen on YouTube Music ]    <- button 1
 *   [ Visit NUViA              ]   <- button 2
 * ```
 */
class DiscordRPC(
    val context: Context,
    ...
    suspend fun updateSong(
        ...
        // Button 2 is reserved for future official NUViA destination (currently none)
     ...
     * a side-by-side dev install should still look like NUViA to everyone
     * else on Discord.
     */
    ...
    companion object {
        ...
        const val DEFAULT_BUTTON_2 = "Visit NUViA"
