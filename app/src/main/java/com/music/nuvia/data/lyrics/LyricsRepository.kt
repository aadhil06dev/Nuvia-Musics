 ...
 *
 * Seven sources, tried in [order] — the user's own priority list in Settings,
 ...
 * lyrics anywhere would spend the best part of a minute finding that out.
 * Run together, a miss costs whatever the slowest one needed to still be waited on took.
 *
...
object LyricsRepository {
    ...
    suspend fun lyrics(
        ...
        val sequence = (order.filter { it in sources } +
            LyricsSource.entries.filter { it in sources && it !in order }).distinct()
        if (sequence.isEmpty()) return@coroutineScope null
        ...
        val racing: List<Pair<LyricsSource, Deferred<List<LyricLine>?>>> = sequence.map { source ->
            source to async(Dispatchers.IO) { fetch(source, videoId, searchTitle, searchArtist, durationMs, album) }
        }
    ...
    private suspend fun fetch(
        ...
        LyricsSource.BINI_LYRICS -> BiniLyrics.lyrics(title, artist, durationMs, album)?.lines
