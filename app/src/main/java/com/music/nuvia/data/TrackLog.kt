 ...
 * NUViA to access all device logs?"* — which appears whenever the process
 ...
 * to logcat as well, so `adb logcat -s NUViA` is unchanged.
 */
object TrackLog {
    ...
    private fun header(
        ...
        appendLine("NUViA log — ${song.title} — ${song.artist}")
