 ...
 *
 *  - `NUVIA_LYRICS` (and legacy `BITCHORD_LYRICS`), this app's own, holding the
 *    "enhanced" A2 form with the word timings intact — see [toEnhancedLrc].
...
object EmbeddedLyrics {
     ...
     * four-byte type *is* the name. They are stored differently and found
     * differently, but both terminate in a standard iTunes `data` atom.
     */
    private fun ilstText(bytes: ByteArray, from: Int, endExclusive: Int, freeform: Boolean): String? {
        val markers = if (freeform) {
            listOf(
                WORD_LYRICS_FIELD.toByteArray(Charsets.UTF_8),
                "BITCHORD_LYRICS".toByteArray(Charsets.UTF_8),
            )
        } else {
            listOf(LYR_ATOM)
        }
