...
import com.music.nuvia.data.model.NOTIFICATION_ART_PX
...
fun rememberPlayerState(controller: MediaController?): PlayerState {
    ...
    DisposableEffect(controller) {
        ...
        fun sync(events: Player.Events? = null, error: String? = null) {
            ...
            // Only reconstruct the queue list when the timeline actually changed or the count differs.
            val timelineChanged = events == null || events.contains(Player.EVENT_TIMELINE_CHANGED)
            val newQueue = if (timelineChanged || state.queue.size != count) {
            ...
            if (song != null && isPlaying) {
                com.music.nuvia.data.history.LocalHistoryStore.record(song)
            }
            if (state != nextState) {
                state = nextState
            }
        ...
        val listener = object : Player.Listener {
            override fun onEvents(p: Player, events: Player.Events) = sync(events, state.error)
            override fun onPlayerErrorChanged(error: androidx.media3.common.PlaybackException?) {
                sync(null, error?.let { "Playback failed: ${it.errorCodeName}" })
            }
        }
...
/**
 * Observes the live playback position from [MediaController] independently of [PlayerState],
 * updating every [pollIntervalMs] while playing.
 *
 * Scoped separately so high-frequency position ticks do not invalidate parent composables
 * (such as [MainActivity] or navigation destinations) that do not render real-time progress.
 */
@Composable
fun rememberPlaybackPosition(
    ...
    val positionState = remember {
        mutableLongStateOf(controller?.currentPosition?.coerceAtLeast(0L) ?: 0L)
    }
    ...
    LaunchedEffect(controller, isPlaying) {
        while (controller != null && isPlaying) {
            positionState.longValue = controller.currentPosition.coerceAtLeast(0L)
 ...
 */
private const val EXTRA_FROM_AUTOPLAY = "nuvia.fromAutoplay"

/** @see Song.localUri */
private const val EXTRA_LOCAL_URI = "nuvia.localUri"

/** @see Song.localPath */
private const val EXTRA_LOCAL_PATH = "nuvia.localPath"
 ...
 * playback URI.
 */
private const val EXTRA_DURATION = "nuvia.durationText"
 ...
 * catalogue audio release by [com.music.nuvia.data.YtMusicRepository.resolveAudio]
 ...
 * terms [com.music.nuvia.data.sources.TrackMatcher] compares recordings on.
 *
...
fun Song.toMediaItem(): MediaItem {
    ...
    val uriString = localUri ?: when {
        ...
        // reach.
        else -> "nuvia://watch?v=$videoId${matchQuery()}"
    }
 ...
 * — see [com.music.nuvia.data.TrackLog.about].
 *
