package com.music.nuvia.playback.smart
 ...
 * [com.music.nuvia.playback.AudioCache], because that is where *streamed*
 * audio lands. A track playing off the device never goes there:
 * [com.music.nuvia.playback.AudioCache.playbackFactory] routes `file://` and
 ...
 * [com.music.nuvia.playback.AudioCache.requestAnalysisHead] — the fallback
...
internal object LocalAudioSource {
    ...
    private const val TAG = "NUViALocalAudio"

    /** Whether [uri] names a file on the device rather than something to fetch. */
    fun isLocal(uri: Uri): Boolean = isLocalScheme(uri.scheme)

    /** Whether [scheme] represents a local file or content URI rather than a remote stream. */
    fun isLocalScheme(scheme: String?): Boolean = when (scheme?.lowercase(Locale.ROOT)) {
        "file", "content" -> true
        else -> false
    }
