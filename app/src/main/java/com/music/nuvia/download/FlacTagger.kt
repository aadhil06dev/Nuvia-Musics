...
object FlacTagger {
    ...
    fun tag(
        ...
        rewrite(bytes, title, artist, album, cover, coverMime, lyrics)
    }.getOrDefault(bytes)
    ...
    private fun vorbisComment(
        ...
        val fields = buildList {
            ...
            if (lyrics != null) {
                if (lyrics.plain.isNotBlank()) add("LYRICS=${lyrics.plain}")
                if (!lyrics.enhanced.isNullOrBlank()) add("${com.music.nuvia.data.lyrics.WORD_LYRICS_FIELD}=${lyrics.enhanced}")
            }
        }
    ...
    private const val VENDOR = "NUViA"
}
