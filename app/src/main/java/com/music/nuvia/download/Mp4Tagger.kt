...
object Mp4Tagger {
    ...
    fun tag(
        ...
        if (lyrics != null) {
            if (lyrics.plain.isNotBlank()) items += textItem("©lyr", lyrics.plain)
            if (!lyrics.enhanced.isNullOrBlank()) items += freeformItem(com.music.nuvia.data.lyrics.WORD_LYRICS_FIELD, lyrics.enhanced)
        }
    ...
    private fun meanAtom(mean: String): ByteArray {
        val body = ByteArray(4 + mean.length)
        mean.toByteArray(Charsets.UTF_8).copyInto(body, 4)
        return box("mean", body)
    }

    private fun nameAtom(name: String): ByteArray {
        val body = ByteArray(4 + name.length)
        name.toByteArray(Charsets.UTF_8).copyInto(body, 4)
        return box("name", body)
    }

    private fun freeformItem(name: String, text: String): ByteArray {
        val mean = meanAtom("com.apple.iTunes")
        val nameBox = nameAtom(name)
        val data = dataAtom(1, text.toByteArray(Charsets.UTF_8))
        return box("----", mean + nameBox + data)
    }
