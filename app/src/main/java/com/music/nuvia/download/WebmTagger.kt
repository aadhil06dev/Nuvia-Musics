...
object WebmTagger {
    ...
    fun tag(
        ...
        insert(bytes, buildTail(title, artist, album, cover, coverMime, lyrics))
    }.getOrDefault(bytes)
    ...
    private fun buildTail(
        ...
        if (lyrics != null) {
            if (lyrics.plain.isNotBlank()) simple += simpleTag("LYRICS", lyrics.plain)
            if (!lyrics.enhanced.isNullOrBlank()) simple += simpleTag(com.music.nuvia.data.lyrics.WORD_LYRICS_FIELD, lyrics.enhanced)
        }
